...
public class ConsolidatedHTMLReport {
	...
	private final Logger reportLog = Logger.getLogger(ConsolidatedHTMLReport.class);
		...
		cons_print_out
				.println(
						"<tr><td class=\"left-active-color\">Application Environment</td><td class=\"right-active-color\">"
								+ ConfigUtil.envConfig().getProperty("Base_URL")
										.replaceAll("(https*://)(\\w+-*\\w+)(.*)", "$2").trim().toUpperCase()
								+ "</td></tr>");
